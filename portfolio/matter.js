...
// Closing Button Java Script is Here

const aboutMeButton = document.getElementById('aboutMeButton');
const aboutMeModal = document.getElementById('aboutMeModal');
const modalContent = aboutMeModal.querySelector('.modal-content');
const closeModal = document.getElementById('closeModal');

aboutMeButton.addEventListener('click', () => {
  const buttonRect = aboutMeButton.getBoundingClientRect();
  const buttonX = buttonRect.left + buttonRect.width / 2 - window.innerWidth / 2;
  const buttonY = buttonRect.top + buttonRect.height / 2 - window.innerHeight / 2;

  modalContent.style.setProperty('--button-x', `${buttonX}px`);
  modalContent.style.setProperty('--button-y', `${buttonY}px`);
...
closeModal.addEventListener('click', () => {
  modalContent.style.animation = 'shrinkIn 0.5s ease forwards';
  modalContent.addEventListener('animationend', () => {
    aboutMeModal.classList.remove('show');
    modalContent.style.animation = '';
    aboutMeButton.classList.remove('hidden');
  }, { once: true });
});
